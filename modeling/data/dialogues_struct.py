...
from modeling.models.convosense_model.utils import add_speaker_to_lines
...
class Turn:
    ...
    silver_cs: dict[str, str] = field(default_factory=dict)
    silver_beam_cs: dict[str, list[str]] = field(default_factory=dict)
...
class Dialogue:
    ...
    def context(self, turn_id=None, return_string=True, with_pointers=False, speaker_tag=None, listener_tag=None):
        if turn_id is None:
            turn_id = len(self.turns) - 1
        return add_speaker_to_lines(
            [t.utt for t in self.turns[:turn_id + 1]],
