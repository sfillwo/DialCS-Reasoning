...
def add_speaker_to_lines(lines: list[str], return_string=True, with_pointers=False, speaker_tag=None, listener_tag=None) -> str:
    if not speaker_tag:
        speaker_tag = 'Speaker' if not with_pointers else 'Speaker (Other)'
    if not listener_tag:
        listener_tag = 'Listener' if not with_pointers else 'Listener (You)'
    elements = list(
        reversed(
            [
                f"{speaker_tag if i % 2 == 0 else listener_tag}: {u}"
